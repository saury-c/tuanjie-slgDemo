...
public class PoisonAttackCard : CardItem, IPointerDownHandler
{
            ...
            if (Input.GetMouseButtonDown(0))
            {
                ...
                if (TryUse())
                {
                    ...
                    if(hitEnemy.Defend + hitEnemy.CurHp > val)
                    {
                        ...
                        {
                            hitEnemy.gameObject.GetComponent<PoisonDebuff>().AddDamage(int.Parse(vals[1]));
                        }
                        else
                        {
                            hitEnemy.gameObject.AddComponent<PoisonDebuff>().Init(hitEnemy,int.Parse(vals[1]));
                        }
                    }
